...
import { SERVICE_DETAILS_ES } from "@/lib/services-es";
import { cn } from "@/lib/utils";

const ICONS: Record<string, LucideIcon> = {
  CalendarCheck, Clock, Sparkles, Wallet, CalendarClock, Wind, HardHat, Building2, Leaf,
  ScanSearch, Home, ShieldCheck, Users, BadgePercent, CalendarDays, Briefcase,
  ClipboardCheck, Baby, Recycle, Zap, Heart, RefreshCw, Target, DollarSign, Plus,
  Headphones, Award, BadgeCheck,
};

const ACCENT_STYLES = {
  crimson: { grad: "from-[#E53935] to-[#C62828]", soft: "bg-crimson-soft", text: "text-[#C62828]", ring: "ring-[#C62828]/20" },
  navy: { grad: "from-[#1A237E] to-[#0D1642]", soft: "bg-navy/5", text: "text-[#1A237E]", ring: "ring-[#1A237E]/20" },
  purple: { grad: "from-[#6A1B9A] to-[#4A148C]", soft: "bg-purple-soft", text: "text-[#6A1B9A]", ring: "ring-[#6A1B9A]/20" },
  gold: { grad: "from-[#D4AF37] to-[#9B7B0E]", soft: "bg-gold-soft", text: "text-[#9B7B0E]", ring: "ring-[#D4AF37]/30" },
};
...
type LocalizedService = ServiceDetail & {
  name: string;
  shortName: string;
  tagline: string;
  summary: string;
...
export function ServiceDetailPage({
  ...
  // Build localized service content
  const es = SERVICE_DETAILS_ES[service.slug];
  const ls: LocalizedService = locale === "es" && es
    ? { ...service, ...es }
    : service;
    ...
    <div className="bg-background">
      ...
      <section className="relative overflow-hidden pt-32 pb-16 sm:pt-40 lg:pt-44 lg:pb-20">
        <div className="pointer-events-none absolute inset-0 -z-10">
          ...
          <div className={cn("absolute -top-24 right-[-10%] h-[480px] w-[480px] rounded-full blur-3xl opacity-60", accent.soft)} />
          <div
            className="absolute inset-0 opacity-[0.3]"
            style={{
              backgroundImage: "radial-gradient(circle at 1px 1px, rgba(26,35,126,0.08) 1px, transparent 0)",
        ...
        <div className="mx-auto w-full max-w-7xl px-5 sm:px-8 lg:px-10">
          <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="mb-8">
            <Link href="/#services" className="group inline-flex items-center gap-1.5 text-sm font-semibold text-muted-foreground transition-colors hover:text-crimson">
          ...
          <div className="grid grid-cols-1 items-center gap-10 lg:grid-cols-2 lg:gap-14">
            <div>
              <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5 }} className="flex flex-wrap items-center gap-3">
                <span className={cn("inline-flex items-center gap-2 rounded-full px-4 py-1.5 text-[0.7rem] font-bold uppercase tracking-[0.18em] text-white bg-gradient-to-br", accent.grad)}>
              ...
              <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.24 }} className="mt-6 flex flex-wrap gap-3">
                {service.priceFrom ? <MetaChip icon={Wallet} label={service.priceFrom} /> : null}
                {service.duration ? <MetaChip icon={Clock} label={service.duration} /> : null}
              </motion.div>

              <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.5, delay: 0.3 }} className="mt-8 flex flex-col gap-3 sm:flex-row">
                <CtaPrimary href={`/book?service=${service.slug}`} icon={CalendarCheck}>{t("nav.book")}</CtaPrimary>
                <CtaSecondary href={SITE.phoneHref} icon={Phone}>{SITE.phone}</CtaSecondary>
              </motion.div>
            </div>

            <motion.div initial={{ opacity: 0, scale: 0.96, y: 24 }} animate={{ opacity: 1, scale: 1, y: 0 }} transition={{ duration: 0.7, delay: 0.2, ease: [0.22, 1, 0.36, 1] }} className="relative">
              ...
              <div className="relative overflow-hidden rounded-[2rem] shadow-[0_30px_80px_-30px_rgba(13,22,66,0.4)] ring-1 ring-navy/5">
                <div className="relative aspect-[4/3] w-full">
                  <Image src={service.image} alt={`${ls.name}`} fill priority sizes="(max-width: 1024px) 100vw, 50vw" className="object-cover" />
                </div>
        ...
        <div className="mx-auto w-full max-w-7xl px-5 sm:px-8 lg:px-10">
          <div className="grid grid-cols-1 gap-12 lg:grid-cols-[1fr_1.1fr] lg:gap-16">
            <div>
              ...
              <Reveal delay={0.15}>
                <div className="mt-8">
                  <p className="text-sm font-bold uppercase tracking-wider text-navy">{t("sd.bestFor")}</p>
                  <div className="mt-3 flex flex-wrap gap-2">
                    {ls.bestFor.map((item) => (
                      <span key={item} className={cn("inline-flex items-center gap-1.5 rounded-full px-3.5 py-1.5 text-sm font-medium", accent.soft, accent.text)}>
            ...
            <Reveal delay={0.1}>
              <div className="relative overflow-hidden rounded-3xl border border-navy/5 bg-white p-6 shadow-[0_20px_60px_-30px_rgba(13,22,66,0.3)] sm:p-8">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-heading text-xl font-bold text-navy">{t("sd.includesTitle")}</h3>
                    <p className="text-sm text-muted-foreground">{t("sd.includesSub")}</p>
                  </div>
                </div>
                <ul className="mt-6 grid grid-cols-1 gap-3 sm:grid-cols-2">
                  {ls.includes.map((item) => (
                    <li key={item} className="flex items-start gap-2.5 rounded-xl bg-muted/40 p-3 text-sm font-medium text-navy">
                ...
                <div className="mt-6 flex flex-wrap items-center gap-3 border-t border-navy/5 pt-5">
                  <CtaPrimary href={`/book?service=${service.slug}`} size="md" icon={CalendarCheck}>{t("sd.bookNow")}</CtaPrimary>
                  <span className="text-sm text-muted-foreground">{t("sd.freeQuote")}</span>
                </div>
              </div>
            </Reveal>
          </div>
              ...
              return (
                <StaggerItem key={step.title}>
                  <div className="group relative flex flex-col items-center text-center">
                    ...
                    <h3 className="mt-6 font-heading text-xl font-bold text-navy">{step.title}</h3>
        ...
        <section className="relative py-16 sm:py-20 bg-gradient-to-b from-gold-soft/30 to-white">
          <div className="mx-auto w-full max-w-7xl px-5 sm:px-8 lg:px-10">
            <div className="text-center">
              ...
              <Reveal delay={0.05}>
                <h2 className="mx-auto mt-5 max-w-2xl font-heading text-3xl font-extrabold tracking-tight text-navy sm:text-4xl">
                  {ls.shortName} {t("sd.galleryTitle")}
                </h2>
              </Reveal>
            </div>
            <Stagger className="mt-12 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3" stagger={0.08}>
              {service.gallery.map((img, i) => (
                <StaggerItem key={img}>
                  <div className={cn("group relative overflow-hidden rounded-2xl shadow-[0_10px_36px_-24px_rgba(13,22,66,0.4)] ring-1 ring-navy/5", i === 0 ? "sm:col-span-2 lg:col-span-1" : "")}>
                    <div className="relative aspect-[4/3] w-full">
                      <Image src={img} alt={`${ls.name} ${i + 1}`} fill sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 33vw" className="object-cover transition-transform duration-700 group-hover:scale-110" />
            ...
            <div className="relative px-6 py-12 text-center sm:px-12 sm:py-16">
              ...
              <Reveal delay={0.1}>
                <p className="mx-auto mt-4 max-w-xl text-pretty text-base text-white/75 sm:text-lg">
                  {t("sd.bookTodayDesc")} {SITE.tagline}.
                </p>
              </Reveal>
              <Reveal delay={0.15}>
                <div className="mt-8 flex flex-col items-center justify-center gap-3 sm:flex-row">
                  <CtaPrimary href={`/book?service=${service.slug}`} icon={CalendarCheck} variant="gold">{t("nav.book")}</CtaPrimary>
                  <CtaSecondary href={SITE.phoneHref} icon={Phone} className="!border-white/30 !bg-white/10 !text-white hover:!bg-white hover:!text-crimson hover:!border-white">{SITE.phone}</CtaSecondary>
                </div>
              </Reveal>
            </div>
          </div>
        ...
        <section className="relative py-16 sm:py-20 bg-gradient-to-b from-white to-gold-soft/30">
          <div className="mx-auto w-full max-w-7xl px-5 sm:px-8 lg:px-10">
            <div className="flex flex-col items-start justify-between gap-4 sm:flex-row sm:items-end">
              ...
              <Reveal delay={0.1}>
                <Link href="/#services" className="group inline-flex items-center gap-1.5 text-sm font-bold text-crimson transition-colors hover:text-crimson-dark">
                  {t("sd.viewAll")}
            ...
            <Stagger className="mt-10 grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-3" stagger={0.08}>
              {related.map((rel) => {
                ...
                const relName = locale === "es" && SERVICE_DETAILS_ES[rel.slug]
                  ? SERVICE_DETAILS_ES[rel.slug].name
                  : SERVICE_NAME_KEY[rel.slug] ? t(SERVICE_NAME_KEY[rel.slug]) : rel.name;
                const relSummary = locale === "es" && SERVICE_DETAILS_ES[rel.slug]
                  ? SERVICE_DETAILS_ES[rel.slug].summary
                  : rel.summary;
                return (
                  <StaggerItem key={rel.slug}>
                    <Link href={`/services/${rel.slug}`} className="group relative flex h-full flex-col overflow-hidden rounded-3xl border border-navy/5 bg-white shadow-[0_10px_40px_-24px_rgba(13,22,66,0.3)] transition-all duration-500 hover:-translate-y-1.5 hover:shadow-[0_28px_60px_-30px_rgba(26,35,126,0.4)]">
                      <div className="relative aspect-[16/11] w-full overflow-hidden">
                        <Image src={rel.image} alt={relName} fill sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, 33vw" className="object-cover transition-transform duration-700 group-hover:scale-110" />
                      ...
                      <div className="flex flex-1 flex-col p-5">
                        <h3 className="font-heading text-lg font-bold text-navy transition-colors group-hover:text-crimson">{relName}</h3>
                        <p className="mt-1.5 flex-1 text-sm leading-relaxed text-muted-foreground">{relSummary}</p>
...
function FaqItem({ question, answer }: { question: string; answer: string }) {
  ...
  return (
    <div className="overflow-hidden rounded-2xl border border-navy/8 bg-white">
      <button type="button" onClick={() => setOpen((o) => !o)} className="flex w-full items-center justify-between gap-4 px-5 py-4 text-left" aria-expanded={open}>
        <span className="font-heading text-base font-bold text-navy sm:text-lg">{question}</span>
        <span className={cn("grid size-8 shrink-0 place-items-center rounded-full transition-all", open ? "bg-gradient-to-br from-[#E53935] to-[#C62828] text-white rotate-180" : "bg-muted text-navy")}>
      ...
      <motion.div initial={false} animate={{ height: open ? "auto" : 0, opacity: open ? 1 : 0 }} transition={{ duration: 0.3, ease: [0.22, 1, 0.36, 1] }} className="overflow-hidden">
        <p className="border-t border-navy/5 px-5 pb-5 pt-4 text-sm leading-relaxed text-muted-foreground">{answer}</p>
      </motion.div>
    </div>
  );
}
